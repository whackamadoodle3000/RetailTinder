...
clothes_data = []
with open('clothes_data.json') as f:
    clothes_data = json.load(f)

display_data = []
with open('display_data.json') as f:
    display_data = json.load(f)


# Example route to handle next image request
@app.route("/next_image", methods=['POST'])
def next_image():
    # Parse JSON data from request body
    request_data = request.get_json()

    # Extract emotion data and swipe direction from JSON
    emotion_data = request_data.get('emotion_data', {})
    swipe_direction = request_data.get('swipe_direction', '')
    clothes_id = request_data.get('clothes_id', '')

    ## use emotion data and user ID to get next image

    next_clothes_id_1 = random.randint(1,69)
    item_1 = next(e for e in clothes_data if e['Clothes_ID'] == next_clothes_id_1)

    next_clothes_id_2 = random.randint(1,69)
    item_2 = next(e for e in clothes_data if e['Clothes_ID'] == next_clothes_id_2)

    next_clothes_id_3 = random.randint(1,69)
    item_3 = next(e for e in clothes_data if e['Clothes_ID'] == next_clothes_id_3)


    response_data = {{
        'next_image': item_1["Image_URL"],
        'clothes_id': next_clothes_id_1,
        'price': item_1["Price"],
        'link_to_shop': item_1["Listing_Link"]
    },
    {
        'next_image': item_2["Image_URL"],
        'clothes_id': next_clothes_id_2,
        'price': item_2["Price"],
        'link_to_shop': item_2["Listing_Link"]
    },
    {
        'next_image': item_3["Image_URL"],
        'clothes_id': next_clothes_id_3,
        'price': item_3["Price"],
        'link_to_shop': item_3["Listing_Link"]
    }}

    return jsonify(response_data)


@app.route("/display_donate", methods=['POST'])
def display_donate():
...
def donate():
    ...
    filename = f"donation_{location}_{random.randint(1,1000)}"+file.filename.split('.')[-1]
    file.save(os.path.join(app.config['UPLOAD_FOLDER'], filename))

    image_descr = get_gpt_response("product_images/"+filename)
    new_id = clothes_data[-1]["Clothes_ID"] + 1
    image_descr["Clothes_ID"] = new_id
    clothes_data.append(image_descr)

    display_data.append({"Clothes_ID": new_id,
      "Type": image_descr["Type"],
      "Image_URL": "product_images/"+filename,
      "Price": image_descr["Price"]})

    # Serialize the object and save it to a JSON file
    with open("clothes_data.json", 'w') as json_file:
        json.dump(clothes_data, json_file, indent=4)

    with open("display_data.json", 'w') as json_file:
