    ...
    <div className={`flex items-center ${className}`}>
      {/* <div className="flex">
        {indicators.map((indicator, i) => {
          const color = indicator ? "bg-blue-500" : "bg-gray-200"; // Different colors for indicators
          return (
            <div
              key={i}
              className={`mr-1 resize-none rounded border border-gray-300 pl-5 pt-5 text-sm text-white shadow ${color}`}
            ></div>
          );
        })}
      </div> */}
